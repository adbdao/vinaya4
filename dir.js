...
for (var i = 0; i < k.length; i++) {
    if (/var ArrLi\s?\=/.test(k[i])) {
        ...
        // 刪除不需要的檔
        k[i] = k[i].replace(',".git"', '').replace(',"index.html"', '').replace(',"dirToIndex.js"', '').replace(',"dir.js"', '').replace(',"debug.log"', '').replace(',"desktop.ini"', '').replace('"老和尚的蓮花.ico",', '').replace(',".gitignore"', '')
    }
}
